...
class RegistrarDescarteView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        descarte.pontos_gerados = int(descarte.peso_kg * 10)
        descarte.save()

        self.request.user.saldo_pontos += descarte.pontos_gerados
        self.request.user.save(update_fields=['saldo_pontos'])

        messages.success(
            self.request,
            f'Descarte registrado! Voce ganhou {descarte.pontos_gerados} pontos.'
        )
